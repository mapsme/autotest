...
export default class ComparingPowerView extends JetView{
	config(){
        ...
        document.title = "Compare memory tests";
		var form = {
	...
	var test_res_list_template = {
            ...
            columns: [
                ...
                {id:"average", name: "average", width:180, header: "Average memory Mb"},
                {id:"time", name: "time", fillspace: true, header: "Time, m"}
            ],
            on:{
        ...
        function wrap_test_results(id){
            $$(id).eachRow(function(row){
                var record = $$(id).getItem(row);
                webix.ajax().get(beta_url + "/average/memory/" + record.id,  function(text, data){
                    var info = data.json()
                    $$(id).mapCells(record.id,"average",1,1,
                        function(value, row_id, column_id, row_ind, col_ind){
                            return info.average_memory;
                        }
                    );
                    $$(id).refresh();
                 });
                        ...
                        if (valid && !valid2) {
                            ...
                            webix.ajax().get(beta_url + "/hardwaretestresults", {"device_id": $$("device").getValue(), "test_item_id": $$("second_test").getValue(),
                             ...
                             $$("go").attachEvent("onItemClick", function(){
                                webix.ajax().bind(this).post(beta_url + "/jsonmetrics/memory", {"first_ids": get_chosen_values("test_result_list1"),
