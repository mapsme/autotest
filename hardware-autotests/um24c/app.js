...
program
	...
	.option("-t, --testid [10]",                 "Test result id")
	.option("-s, --server [100]", "server host")
	.parse(process.argv);
...
var convertData = function(buffer) {
	...
	//we dont need groups because of reset!!
	/*for(var g=0; g<10; g++) {
	    if (g == data.group){
		data.groups[g] = {
			// Current in "mAh"
...
var connect = function(address, name) {
	device.findSerialPortChannel(address, function(channel) {
		...
		bluetooth.connect(address, channel, function(err, connection) {
			...
			connection.write(new Buffer("f4", "hex"), function() {}); //to reset group
...
if(program.address) {
	...
	console.error("Searching for UM24C device...");
