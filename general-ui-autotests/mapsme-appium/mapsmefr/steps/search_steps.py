...
class AndroidSearchSteps(SearchSteps, AndroidSteps):
    ...
    def assert_shops_list(self):
        sights = [LocalizedCategories.MALL.get(), LocalizedCategories.CAFE.get(),
                  ...
                  LocalizedCategories.WINE_SHOP.get(), LocalizedCategories.BUTCHERS.get(),
                  LocalizedCategories.HAIRDRESSER.get(), LocalizedCategories.HARDWARE_STORE.get(),
                  LocalizedCategories.SWEETS.get(), LocalizedCategories.PETSHOP.get(),
                  LocalizedCategories.SHOE_STORE.get()
                  ]
        for _ in range(3):
            results = [x.text for x in self.driver.find_elements_by_id("description")]
            for res in results:
